...
export default function ModelSelectModal({
  ...
  const fetchEnabledModels = async () => {
    try {
      ...
      if (!res.ok) throw new Error(`Failed to fetch enabled models: ${res.status}`);
      const data = await res.json();
      setEnabledModels(data.enabled || {});
    } catch (error) {
  ...
  const groupedModels = useMemo(() => {
    ...
    return groups;
  }, [filteredActiveProviders, modelAliases, allProviders, providerNodes, customModels, enabledModels, kindFilter, activeProviders, cursorModels, clineModels, clinepassModels, nineRouterModels]);
