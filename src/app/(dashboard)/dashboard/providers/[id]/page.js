...
export default function ProviderDetailPage() {
  ...
  const usesLiveModelCatalog = providerId === "cursor" || providerId === "9router" || providerId === "zed";
  ...
  // Live per-connection catalogs (cursor, 9router, zed): these providers expose
  // account-specific model catalogs that change frequently. Load the active
  // account's live catalog for the dashboard; the static registry remains the
  // fallback while the request is pending or unavailable. Fires only when the
  // provider id or connection list changes — no polling, no loop.
  // Cursor/9router path is unchanged; zed adds error surfacing.
  useEffect(() => {
  ...
  }, [providerId, connections, usesLiveModelCatalog]);
